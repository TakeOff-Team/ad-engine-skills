...
| `annotated-hero` | taste-driven "made at a desk" hero (from Remy's thumbnails) | `headline` (ONE `<em>` keyword) `headline-face` (`display` · `body`) `object` (img) or `object-html` (authored: `.file-card`, `.stack`, `.file-card.body` helpers) `object-rotate` `object-width` `note` `note-pos` (`br` · `tr` · `bl` · `tl`) `arrow` (`on`) `mark` (`asterisk`) `founder` (transparent PNG) `founder-height` `cta` `disclaimer` `theme` (`""` dots · `lines` · `plain` · `ink`) | exactly one `<em>` enforced; empty stage flagged; non-PNG founder flagged; >6 words flagged. Scribble underline drawn in `__afterFit` from the fitted `<em>` rects |

`assets/inspiration/` → `node inspiration.js <brand-dir | slug> [--paper]` builds the taste board (contact sheet + index, optional Paper Inspiration page). `render.js` warns `FONT` when a brand face didn't load (measured after an explicit `document.fonts.load`, so locally installed faces count).
